!(function () {
	define('BrandVarInfoModel', function (require, exports, module) {
		var Model=function(){
			var _Self = this;
			//创建canvas对象模块
			...
			function InitUI(){
				...
				_MoviePanel=new varLib.VarInfoSkin();
				_CJSModel.Root.addChild(_MoviePanel);
				_Panel=_MoviePanel.panel;
				
				
				touch.on( "#BrandVar", 'swipedown',toLand);
				touch.on( "#BrandVar", 'swipeup',toTast);
				
				function toLand(){
					touch.off( "#BrandVar", 'swipedown',toLand);
					touch.off( "#BrandVar", 'swipeup',toTast);
					var _obj={v:-1100,t:0}
					JT.to(_obj,1,{delay:.5,v:0,t:1100,
						onUpdate:function(){
							$("#BrandVar").css("top",_obj.t);
